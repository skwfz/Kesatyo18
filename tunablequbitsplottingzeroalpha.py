...
pl.plot(Ts,Os,'b')
pl.legend(["Theoretical best fidelity","Other"])
...
data_200SP = np.load('Z:\Documents\\2018Kesätyö\Koodia\Tunable_qubits_data\Couplingalpha_0\squarepulse_200amplitudes_16states_avgfidelity_alpha0.npz')
(TS,OS) = np.meshgrid(data_200SP['times'],s.omega[1]*(1-data_200SP['As']))
pl.figure()
pl.pcolor(TS,OS,data_200SP['avg_fidelities'],vmin=0)
pl.colorbar()
pl.title("Average fidelity of the gate operation with a square pulse, "+r"$\alpha_c$ = 0")
pl.xlabel("Time (s)")
pl.ylabel(r"$\omega_c$")

Ts = np.linspace(0.5e-8,5e-7,100)
As = np.zeros(100)
Tsaccur = np.linspace(1e-7,5e-7,100)
Asaccur = np.zeros(100)
for i in range(0,100):
...
pl.figure()
pl.pcolor(TS,OS,data_200SP['avg_phase_differences'],cmap='seismic', vmin=-np.pi, vmax=np.pi)
pl.colorbar()
pl.title("Average phase difference to the ideal end state "+r"$\alpha_c$ = 0")
pl.xlabel("Time (s)")
pl.ylabel(r"$\omega_c$")
